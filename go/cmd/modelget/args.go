...
// which is a flag's VALUE whenever a flag came first — `modelget get -o D:/m
// hf://org/repo` resolved a model called "D:/m" and wrote it to "hf://org/repo".
func parse(fs *flag.FlagSet, args []string, want ...string) ([]string, error) {
	...
	// An empty value is the same lie as an unknown flag: `-o "$DEST"` with DEST
	// unset would put 40 GB of weights wherever the shell happened to be.
	var blank string
...
// know than a model it could not fetch. See abstraction-download/CONTRACT.md
